...
loop(#state{parent_pid = ParentPid, host=Host} = State) ->
        ...
        {send, Data, Ref}  ->
            case  do_send(State, Data) of
                ...
                {queued, NewState} -> %% we reach the max allowed queued messages.. close the connection.
                    ...
                    ts_mon_cache:add({count, {error_bosh_maxqueued, Host}}),
                    ParentPid ! {ok, Ref},
                ...
                terminate ->
                    if
                        State#state.session_state  /= 'closing' ->
                            ts_mon_cache:add({count, {error_bosh_terminated, Host}}),
        ...
        {Close, Socket} when Close == tcp_closed ; Close == 'ssl_closed' ->
            ...
            case lists:keymember(Socket, 1, State#state.open) of
                true ->
                    ...
                    ts_mon_cache:add({count, {error_bosh_socket_closed, Host}}),
                    State#state.parent_pid ! {gen_ts_transport, self(), closed};
                false ->
...
do_receive_http_response(State, Socket, Vsn) ->
    ...
    ts_mon_cache:add({ sum, {size_rcv, Host}, iolist_size([ [if is_atom(H) -> atom_to_list(H); true -> H end, V] ||
        ...
        _R ->
            ...
            case Content of
                [] ->
                    ...
                    ts_mon_cache:add({ sum, {size_rcv, Host}, iolist_size(Resp)});
                _ ->
...
make_request(Type, Socket,Host, Path, Body, OriginalSize) ->
     ts_mon_cache:add({count, {bosh_http_req, Host}}),
     ...
     ts_mon_cache:add({ sum, {size_sent, Host}, iolist_size(Request) - OriginalSize }).
...
new_socket(State = #state{type = Type, host = Host, port = Port, local_ip = LocalIp, local_port = LocalPort, connect_timeout=Timeout}, Active) ->
    ...
    ts_mon_cache:add({count, {bosh_http_conn, Host}}),
    {State, Socket}.
